...
$$
The Gumbel density and distribution are plotted below alongside the Normal for comparison.
...
Bur of course $\varepsilon_{ik}$ is not given, so to obtain the desired probability $P_{ik}$, we must integrate over all possible values of $\varepsilon_{ik}$, weighted by its marginal density $f_{\varepsilon_{ik}}(\varepsilon_{ik}) = \exp(-\varepsilon_{ik}) \times \exp ( -\exp(\varepsilon_{ik})$:
$$ 
